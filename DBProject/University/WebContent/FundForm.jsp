				...
				if (s.getStudentfunddetail().getFundPetitionStatus() == 0) {
					...
					if ("submit".equals(action)) {
						...
						if (fund != null && !fund.isEmpty()) {
							...
							Integer validation = s.getFamilyIncome().compareTo(
									d.getTuition());
							...
							if ((res == 0 || res == 1)) {
								...
								if (validation == -1) {
									...
									sfnd.setFundPetitionStatus(1);
									sfnd.setFundRequired(fundRequired);
									sfnd.setPetitionDescription(message);

									sfnd = sfnddao.updateStudentFundDetails(s.getStudentId(),sfnd);
