<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
	...
	import="edu.neu.cs5200.university.model.*,edu.neu.cs5200.university.dao.*,java.util.*"%>
						...
						<form action="UniversityLogin.jsp" class="form-horizontal">
							<%
								...
								if ("login".equals(action)) {
									...
									if (p != null) {
										if (p.getRoleName().matches("student")) {

											session.setAttribute("personId", p.getPersonId());
											session.setAttribute("userName", p.getUserName());
											response.sendRedirect("StudentLanding.jsp"); //for real
										}

										else if (p.getRoleName().matches("admin")) {
											session.setAttribute("personId", p.getPersonId());
											session.setAttribute("userName", p.getUserName());
											response.sendRedirect("AdminLanding.jsp"); // for real
										} else {
							%>
